...
function displayProductDetails(productId) {
  ...
  if (product) {
    const productImages = document.querySelector(".product-images img");
    const productSubImages = document.querySelectorAll(
    ...
    productTotalAmount.value = `₱${
      parseInt(productQuantity.value) * product.price
    }`;
    ...
    addToCartButton.addEventListener("click", (e) => {
      ...
      alert(`${total} x ${product.title} added to cart!`);
    });
    console.log(productId);
    // Display similar products based on category
    displaySimilarProducts(product.category, productId);
  } else {
